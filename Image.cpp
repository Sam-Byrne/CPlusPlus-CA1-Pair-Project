...
void MyImage::greyScale() {
    ...
    for (RGB & pixel: pixels) {
        ...
        unsigned char  average = ((0.3 * pixel.r ) + (0.59 * pixel.g) + (0.11 * pixel.b));
...
void MyImage::flipHorizontal() {
    ...
    for (float y = 0; y < size.y; ++y) {
        for (float x = 0; x < size.x/2; ++x) {
            ...
            RGB pixel = pixels[leftPixel];

            pixels[leftPixel] = pixels[rightPixel];
            pixels[rightPixel] = pixel;
...
void MyImage::flipVertical() {
    ...
    for (float y = 0; y < size.y/2; y++) {
        for (float x =0; x < size.x;x++) {

            int topPixel = y * size.x + x;
            int bottomPixel = (size.y - 1 - y) * size.x + x;
            RGB pixel = pixels[bottomPixel];

            pixels[bottomPixel] = pixels[topPixel];
            pixels[topPixel] = pixel;
